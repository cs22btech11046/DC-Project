// simplified late-binding scheduler (no cancellation)
#include <arpa/inet.h>
#include <unistd.h>
#include <vector>
#include <chrono>
...
int rpc(string ip, int port, string msg, string &reply) {
    ...
    if(n<=0){close(s);return-2;}
    buf[n]=0; reply=buf; close(s); return 0;
}
...
int main(){
    ...
    int jobs=100;
    long total_wait=0,total_service=0,total_resp=0,total_rpc=0;
    ...
    for(int j=0;j<jobs;j++){
        ...
        // Late binding: ask workers to CALL US when ready
        for(auto &w:W){
            string r;
    ...
    cout<<"Avg service: "<<total_service/jobs<<" ms\n";
    cout<<"Avg response: "<<total_resp/jobs<<" ms\n";
    cout<<"Avg RPC per job: "<<total_rpc/jobs<<"\n";
}
